...
class MinHeap( Heap ):
    """Specialized form of the abstract Heap.
       ...
       a Node is always smaller (by value) than
       its children, there is still no connection
       between its children. This has the 
       advantage that the root of this tree is
       ALWAYS THE GLOBAL MINIMUM of the entire Heap."""

    def __init__( self ):
        """Creates a new but empty MinHeap."""
    ...
    def insert( self, value ):
        ...
        node = Node( value )
        super( MinHeap, self ).insert( node )
    ...
    def deleteMin( self ):
        """Deletes the minimum Node that is
    ...
    def __bubbleDown( self, node ):
        """Performs a 'bubbleDown' operation, so it
           ...
           caused by the 'deleteMin' operation. 
